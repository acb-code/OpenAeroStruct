"""oas-cli — Command-line interface for the OAS MCP tools.
...
Interactive mode::
    ...
    echo '{"tool":"create_surface","args":{"name":"wing","num_y":7}}
    {"tool":"run_aero_analysis","args":{"surfaces":["wing"],"alpha":5}}' \\
      | oas-cli interactive

One-shot mode::

    oas-cli create-surface --name wing --num-y 7
    oas-cli run-aero-analysis --surfaces '["wing"]' --alpha 5
...
def oneshot_mode(
    ...
    from oas_mcp.cli_runner import run_tool, get_registry, json_dumps
    from oas_mcp.cli_state import load_surfaces, save_surfaces
    ...
    if tool_name == "reset" and response.get("ok"):
        from oas_mcp.cli_state import clear_state
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="oas-cli",
        description="OpenAeroStruct CLI — run OAS tools from the command line.",
    )
    parser.add_argument(
    ...
    # We always build them here for argparse help/completion.
    try:
        from oas_mcp.cli_runner import get_registry
