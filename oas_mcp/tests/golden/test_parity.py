...
Run with pytest:
    pytest oas_mcp/tests/golden/test_parity.py -v
    pytest -m parity

Run directly to see a side-by-side comparison of OAS vs MCP results:
    python oas_mcp/tests/golden/test_parity.py
"""
...
_PARITY_PATH = Path(__file__).parent / "parity_values.json"
...
async def _mcp_rect_aero() -> dict:
    ...
    from oas_mcp.server import create_surface, reset, run_aero_analysis
    await reset()
...
async def _run_parity_report() -> int:
    ...
    for i, case in enumerate(cases, 1):
        ...
        hdr = (
            ...
            f"{'MCP Result':<{col_w}}  {'Δ rel':<12}  Status"
        )
        print()
        print(hdr)
        print("  " + "-" * (len(hdr) - 2))
