"""OIDC Authorization Code flow for the OAS provenance viewer.

When the viewer runs in HTTP mode and Keycloak (or another OIDC provider) is
configured, users authenticate via a browser-based login flow instead of Basic
Auth.  A signed session cookie tracks identity across requests.

Configuration (environment variables):

    OIDC_ISSUER_URL                — OIDC issuer (same as MCP tool auth)
    OAS_VIEWER_OIDC_CLIENT_ID      — viewer OAuth2 client ID (default: "oas-viewer")
    OAS_VIEWER_OIDC_CLIENT_SECRET  — viewer client secret (required for OIDC mode)
    OAS_VIEWER_SESSION_SECRET      — cookie signing key (auto-generated if unset)
    OAS_VIEWER_ADMIN_ROLE          — realm role granting all-user access (default: "oas-admin")
    RESOURCE_SERVER_URL            — public URL of this server (for redirect_uri)
"""
...
def build_viewer_oidc_config() -> ViewerOIDCConfig | None:
    ...
    from oas_mcp.core.auth import _env
