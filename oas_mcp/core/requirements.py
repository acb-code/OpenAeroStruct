"""Requirements checking — dot-path + operator assertion on analysis results.

v1 supports simple dot-path access with comparison operators.
A "v2 direction" supporting wildcards (``surfaces.*.failure.max < 1.0``)
...
-------------
    ...
    report = check_requirements(reqs, results)
...
Path resolution
---------------
Dot-separated keys into the results dict.  Example:
    "CL"                        → results["CL"]
    "surfaces.wing.failure"     → results["surfaces"]["wing"]["failure"]
    "final_results.CD"          → results["final_results"]["CD"]

Future (v2 — not yet implemented)
----------------------------------
    "surfaces.*.failure.max"    wildcard across all surfaces
    "@max(surfaces.*.failure)"  aggregation functions
"""
