...
git clone https://github.com/<your-fork>/OpenAeroStruct.git
cd OpenAeroStruct
...
Verify the install:
```bash
python -c "import openaerostruct; print(openaerostruct.__version__)"
oas-cli --help
```
...
They use OpenMDAO directly with no wrappers. Run them from the repo root:

```bash
# Single-point aero analysis
python openaerostruct/examples/rectangular_wing/run_rect_wing.py

# Drag polar (opens a matplotlib plot at the end)
python openaerostruct/examples/rectangular_wing/drag_polar.py

# Twist optimisation (prints optimizer debug output)
python openaerostruct/examples/rectangular_wing/opt_twist.py

# Chord optimisation
python openaerostruct/examples/rectangular_wing/opt_chord.py
```
...
```bash
cd oas_mcp/demonstrations/rectangular_wing

python lane_a/aero_analysis.py    # prints {"CL": ..., "CD": ...}
python lane_a/drag_polar.py       # prints {"alpha": [...], "CL": [...], "CD": [...]}
python lane_a/opt_twist.py        # prints {"CL": ..., "CD": ..., "twist_cp": [...], ...}
python lane_a/opt_chord.py        # prints {"CL": ..., "CD": ..., "chord_cp": [...], ...}
```
...
Individual scripts:
```bash
oas-cli --pretty run-script lane_b/aero_analysis.json
oas-cli --pretty run-script lane_b/drag_polar.json
oas-cli --pretty run-script lane_b/opt_twist.json
oas-cli --pretty run-script lane_b/opt_chord.json
```

Or run all four with a summary table:
```bash
python lane_b/run_all.py
```
...
```bash
# From the repo root (MCP server auto-discovered)
claude
...
```bash
# From the repo root
pytest oas_mcp/demonstrations/rectangular_wing/tests/ -v
```
