"""SQLite-backed provenance database for the OAS MCP server.

Schema
------
sessions(session_id, notes, oas_session_id, started_at)
tool_calls(call_id, session_id, seq, tool_name, inputs_json, outputs_json,
           status, error_msg, started_at, duration_s)
decisions(decision_id, session_id, seq, decision_type, reasoning,
          prior_call_id, selected_action, confidence, recorded_at)

Thread safety
-------------
A ``threading.local()`` object holds per-thread connections.  WAL mode is
enabled so readers do not block writers.
"""
