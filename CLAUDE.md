...
### Testing oas_mcp changes

After modifying any code under `oas_mcp/`, always:

1. Run the pytest suite: `python -m pytest oas_mcp/tests/ -v`
2. Run golden tests: `python -m pytest oas_mcp/tests/golden/ -v`
3. Run a CLI integration test with `oas-cli run-script` to confirm end-to-end
   behaviour. Write a short JSON script exercising the changed tools, execute
   it, and verify the output. Example:

```bash
oas-cli --pretty run-script my_test.json
```

Clean up any `oas_data/` artifacts generated by CLI test runs afterwards.
